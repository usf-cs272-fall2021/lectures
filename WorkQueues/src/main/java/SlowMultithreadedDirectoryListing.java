	...
	public static Set<Path> list(Path path) throws InterruptedException {
		...
		if (Files.exists(path)) {
			...
			if (Files.isDirectory(path)) {
				...
				/*
				 * Now what? How do we wait for work to be done? Well, the main thread could
				 * call join() on the worker here. But, this becomes non-ideal when we get into
				 * the run() method of our workers...
				 */
		...
		public void run() {
			try (DirectoryStream<Path> stream = Files.newDirectoryStream(path)) {
				for (Path current : stream) {
					 ...
					 *
					 * We generally try to stay away from putting a synchronized block INSIDE of a
					 * loop!
					 */
					synchronized (paths) {
						paths.add(current);
					...
					if (Files.isDirectory(current)) {
						 ...
						 * at a time. Check out the logs to be sure!
						 */
						worker.join();
					}
				}
				 ...
				 * to suppress interrupts entirely, but we are not actively using them yet in
				 * class.
				 */
